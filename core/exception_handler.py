...
def report_unhandled_exception(exctype=None, value=None, traceback=None):
    ...
    print(f"현재 {NAME} 내부에 발생한 오류 전문입니다.\n위의 오류 원인을 제공하면 오류 해결에" " 큰 도움이 됩니다!")

    if (
        QMessageBox.information(
            None,
            f"{NAME} 예외 처리 체계",
            f"죄송합니다. {NAME} 에 내부 오류가 발생했습니다.\n"
        ...
        if traceback is None:
            QMessageBox.critical(
                None, f"{NAME} 예외 처리 체계", f"죄송합니다. {NAME} 에 처리할 수 없는 내부 오류가 발생했습니다.\n"
            )
            sys.exit(-1)
        # configure backends
        qcrash.install_backend(qcrash.backends.GithubBackend(GITHUB_OWNER, GITHUB_REPO))
