...
def file_new():
    return QAction(
        ...
        parent,
        shortcut="Ctrl+N",
        triggered=lambda: QMessageBox.information(None, "0", "0"),
    )
...
def file_save_as():
    return QAction(
        ...
        triggered=lambda: parent.save_as(),
    )
...
def file_save_as_image():
    return QAction(
        ...
        parent,
        shortcut="Ctrl+I",
        triggered=lambda: save_node_as_image(target=parent.editor, parent=parent),
    )
...
def undo():
    return QAction(
        ...
        parent,
        shortcut="Ctrl+Z",
        triggered=lambda: parent.editor.scene.history.undo(),
    )
...
def redo():
    return QAction(
        ...
        triggered=lambda: parent.editor.scene.history.redo(),
    )
...
def copy():
    return QAction(
        ...
        parent,
        shortcut="Ctrl+C",
        triggered=lambda: QApplication.instance()
        .clipboard()
        .setText(
            json.dumps(
                parent.editor.scene.clipboard.serializeSelected(delete=False), indent=4
            )
        ),
    )
...
def _paste():
    try:
        ...
        QMessageBox.warning(
            None, "올바른 JSON 자료가 아닙니다.", "JSON 에서 인식할 수 없는 자료 형식입니다.\n" f"{e}"
        )
        return False
...
def delete():
    return QAction(
        ...
        parent,
        shortcut="Del",
        triggered=lambda: parent.editor.scene.grScene.views()[0].deleteSelected(),
    )
...
def run():
    ...
    return QAction(
        ...
        parent,
        shortcut="Shift+F5",
        triggered=lambda: _save_and_run(CONF["FILE_PATH"]),
    )
...
def compile_to_python():
    return QAction(
        ...
        parent,
        shortcut="Shift+F5",
        triggered=lambda: save_to_py(CONF["FILE_PATH"]),
    )
...
def packaging():
    def _():
        if save_to_py(CONF["FILE_PATH"]):
            _dir, kw = QFileDialog.getExistingDirectory(
                parent,
                f"{NAME} - " f"패키징 폴더 지정",
                os.path.expanduser("~"),
    ...
    return QAction("패키징(&P)...", parent, shortcut="", triggered=lambda: _())
...
def license_dialog():
    return QAction(
        "라이선스", parent, shortcut="", triggered=lambda: OpenSourceLicense(parent)
    )
...
def on_new_leaf(x, y, defined=False):
    ...
    if not defined:
        ...
        #                         ↓TODO: Show(s) the icon of its function.
        exec(
            f'Leaf{leaf_count} = CreateNode(editor.scene, " ", inputs=[0,], outputs=[1], types=defined)'
        )
        exec(f"Leaf{leaf_count}.setPos(x, y)")
    ...
    if parent.editor.scene.clipboard.itemSelected():
        menu.addAction(
            QAction(
                ...
                .setText(
                    json.dumps(
                        parent.editor.scene.clipboard.serializeSelected(delete=False),
                        indent=4,
                    )
                ),
            )
        ...
        menu.addAction(
            QAction(
                "선택한 잎 제거",
                p,
                triggered=lambda: parent.editor.scene.grScene.views()[
                    0
                ].deleteSelected(),
            )
        )
        menu.addAction(
            QAction(
                ...
                .setText(
                    json.dumps(
                        parent.editor.scene.clipboard.serializeSelected(delete=True),
                        indent=4,
                    )
                ),
            )
        ...
        menu.addAction(
            QAction("실행 취소", p, triggered=lambda: parent.editor.scene.history.undo())
        )
        menu.addAction(
            QAction(
                "마지막 작업 다시 실행", p, triggered=lambda: parent.editor.scene.history.redo()
            )
        )
