...
AUTO_GENERATED = (
    f"# AUTO GENERATED BY {NAME.upper()}.\n"
    "# AT {by_class}, {when}\n"
...
class SpriteScriptEditor(PyCodeEdit):
    def __init__(self, parent=None):
        ...
        self.setPlainText(AUTO_GENERATED)

        self.setWindowTitle(f"{NAME} {self.__class__.__name__}")

    def showEvent(self, event):
        self.setWindowTitle(
            f"{NAME} {self.__class__.__name__} - {CONF['FILE_PATH']} [{CONF['CLASS_PATH']}]"
        )
    ...
    def save(self):
        with open(CONF["CLASS_PATH"], "w", encoding="utf8") as _cf:
            if f"# AUTO GENERATED BY {NAME.upper()}." not in self.toPlainText().split(
                ...
                _cf.write(
                    AUTO_GENERATED.format(
                        by_class=self.__class__.__name__, when="UNKNOWN"
                    )
                )
